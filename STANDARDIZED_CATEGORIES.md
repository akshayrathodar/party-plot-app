# Standardized Categories for Party Plots

## 📋 All Possible Categories (11 Total)

1. **Party Plot** - General party plots and lawns
2. **Banquet Hall** - Banquet halls and function halls
3. **Wedding Venue** - Wedding-specific venues
4. **Event Venue** - General event venues and spaces
5. **Resort** - Resorts and resort hotels
6. **Hotel** - Hotels and lodging facilities
7. **Restaurant** - Restaurants, cafes, and food venues
8. **Farm** - Farm venues for events
9. **Community Center** - Community halls and centers
10. **Festival Hall** - Festival and celebration halls
11. **Function Room** - Function rooms and facilities

---

## 🔄 Category Mapping Rules

### Party Plot
- party plot
- party lawn
- party venue
- party planner

### Banquet Hall
- banquet hall
- banquet
- function hall

### Wedding Venue
- wedding venue
- wedding lawn
- wedding hall
- marriage venue
- wedding planner
- wedding service
- wedding buffet
- marriage celebrant

### Event Venue
- event venue
- event hall
- event space
- event planner
- exhibition planner
- meeting planning service
- garden

### Resort
- resort
- resort hotel

### Hotel
- hotel
- lodging

### Restaurant
- restaurant
- fast food restaurant
- coffee shop
- bakery and cake shop

### Farm
- farm

### Community Center
- community center
- community hall

### Festival Hall
- festival hall

### Function Room
- function room facility
- function room

---

## 📊 CSV Category Distribution (After Standardization)

Based on the cleaned CSV data:
- **Wedding Venue**: ~57 occurrences
- **Event Venue**: ~49 occurrences
- **Banquet Hall**: ~10 occurrences
- **Resort**: ~2 occurrences
- **Hotel**: ~3 occurrences
- **Restaurant**: ~4 occurrences
- **Party Plot**: ~3 occurrences
- **Farm**: ~2 occurrences
- **Community Center**: ~1 occurrence
- **Festival Hall**: ~2 occurrences
- **Function Room**: ~3 occurrences

---

## ✅ Notes

- Categories are case-sensitive: Use title case (e.g., "Party Plot" not "party plot")
- Multiple categories can be assigned (comma-separated)
- Default category: "Party Plot" (if category not found in mapping)
- Categories should be added to database via admin panel before CSV import

